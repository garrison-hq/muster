...
describe("checkReferences — LOW-C: an unreadable doctrine subtree must error, not degrade to zero findings", () => {
  it("rethrows when a doctrine subtree exists but is unreadable (EACCES), rather than silently walking as empty", async ({
    ...
    const root = await makeTmpDir();
    await buildDoctrineTree(root);
    const directivesSubtree = join(root, "directives");
    await chmod(directivesSubtree, 0o000);
    try {
      const p = profile({ profileId: "architect-alphonso", directiveRefs: ["001"] });
      await expect(checkReferences([p], root)).rejects.toThrow();
    } finally {
      // Restore permissions so afterEach's recursive rm can clean up.
      await chmod(directivesSubtree, 0o755);
    }
  });
});
