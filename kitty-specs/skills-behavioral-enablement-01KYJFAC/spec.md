...
- The **a2a adapter's** `applyControlInversion` instance is different. Issue #62's own "Blast radius" section explicitly separates what it verified end-to-end (the skills bug above — reproduced twice, against two refs, via a real CLI run) from what it found by code-reading only, not executed (the a2a instance, `src/adapters/a2a/index.ts:356-369`, offered as "worth a maintainer look," explicitly not a verified live repro). The two defects are real and structurally related (both let a discrimination control's fixture-absence read as a pass), but they:
