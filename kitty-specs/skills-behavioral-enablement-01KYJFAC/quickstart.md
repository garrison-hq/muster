...
| `behavioral-weather-skill` — observed trigger rate | should-trigger axis: `0.9667` (29/30 runs); near-miss axis: `0.2` (6/30 runs). **Caveat: neither axis on this case is a semantic tool-fit discrimination signal.** `fixtures/skills/skills-manifest.yaml`'s `behavioral-weather-skill` case targets the `valid/minimal` skill as a placeholder `skillDir` (documented at `fixtures/skills/skills-manifest.yaml:214-226` and mirrored in WP04's own T018 note), whose `SKILL.md` description is generic ("A minimal example skill with only the required fields.") and carries no weather semantics — so both axes here actually measure the model's generic tool-calling propensity on a query, not whether the query fits the tool's description. This is corroborated by the separate `examples/skills/manifest.yaml` weather case (same `valid/minimal` skillDir shape, WP04 T018/T019): its near-miss axis rate was observed to move from `0.63` to `0.03` across two live runs bracketing the MEDIUM-3 remediation commit (`c357219`, "tighten three borderline weather queries"), even though that commit reworded only three `shouldTrigger` queries and left the `nearMiss` query block byte-for-byte unchanged (confirmed via md5) — the shift is not attributable to any committed diff, which is exactly what "unstable generic-propensity axis" predicts and a real semantic-discrimination margin would not do. This instability is pre-existing to this mission and out of WP04's `owned_files`/scope to fix (it would require giving `valid/minimal` — or a dedicated fixture skill — real weather semantics). Do not read future drift on either weather case's near-miss axis, on its own, as a regression without first checking whether the skill's description is still the generic `valid/minimal` placeholder. |
