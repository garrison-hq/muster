...
- **Remaining (non-blocking):**
  1. FR-005 refusal-grader consolidation (Note 1) — import the SOP adapter's exported graders instead of the second in-module implementation before upstreaming.
  2. No distinct `skipped` tally — cases with no `expected` declaration roll into `summary.failed` with a self-documenting per-case reason (conservative fail-safe, never a false green). Consider a separate `skipped` counter for clearer C-004 conformance reporting.
  3. `scenario-second-endpoint.yaml` fixture is owned but not referenced by the master `manifest.yaml` or loaded by name in tests; portability (scenario 12) is instead proven by the integration suite constructing two endpoint configs and spying fetch URLs. Behavior is covered; the standalone fixture is currently illustrative. LOW.
  4. `buildChatClient` apiKeyEnv union cast — widen the core type or document the intentional widening at the seam.
