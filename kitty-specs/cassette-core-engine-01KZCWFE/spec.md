...
muster today has one seam every chat-based command calls through: `ChatClient.chat` /
`ToolChatClient.chatWithTools` (`src/core/behavioral/types.ts:187-192`,
`src/core/behavioral/client.ts:105-111`). Every behavioral run is a live network call —
there is no way to record a real conversation and replay it later without credentials or
network access, and no shared way for a command to know "a recorded execution source is
configured" as opposed to "a live endpoint is configured."
...
(the flagship command, and the only one of muster's seven behavioral runners with no
endpoint skip-gate in front of it); and a **shared execution-source resolver**, exported
from core, that the four gate-bearing commands in wave 2 (#100/#101/#102, plus the
transitively-blocked #103) will consume unmodified.
...
- The other six chat-based commands (`skills run`, `sop run`'s live endpoint probes
  beyond the #90 fix, `crosslayer`, `tools`, `memory`, `heartbeat`) — M17 (#100), M18
  (#101), M19 (#102) wire cassette support into their own gate-bearing CLI surfaces.
...
- Reuse of the unified-report-envelope sanitizer (FR-024 in the report-envelope
  programme, #91/#92). That sanitizer rewrites `apiKey` and regexes every
...
- Any change to the four gate-bearing commands' own `MUSTER_ENDPOINT` skip-gate
  behavior. `behave run` has no such gate today and none is added; the resolver
  ships here so wave 2 can wire its own gates around it later.
...
4. **Given** a previously recorded cassette, **When** run with
...
5. **Given** the same replay run twice, **When** the two verdict `--json` payloads
   are compared, **Then** they are byte-identical.
6. **Given** a k-of-n case (n ≥ 3) whose recorded exchanges share an identical
...
7. **Given** any replay run's report or `--json` payload, **When** inspected,
...
8. **Given** a cassette with one recorded exchange deleted, **When** replayed,
...
14. **Given** `sop run` executed against a real credentialed endpoint, **When** the
...
| FR-004 | Each recorded exchange MUST store a canonical-JSON request hash, a per-key ordinal, the fully-built request (`messages[]`, `opts`), the response, and minimal provenance limited to the model name and the endpoint **hostname only** — never the full URL, never `apiKeyEnv`'s value, never key material. | Proposed |
...
| FR-007 | `makeCassetteClient` MUST decorate both `ChatClient.chat` (`types.ts:187-192`) and `ToolChatClient.chatWithTools` (`client.ts:105-111`) around any concrete client instance, without introducing a new fetch call site. | Proposed |
...
| FR-013 | A replay miss MUST fail only the affected run/case, MUST NOT stop the suite from continuing to its remaining cases, MUST be labeled distinctly as staleness (not generic conformance failure) in the run/case output, and the suite MUST still exit with its normal non-zero failure code — never 0, never a skip code. | Proposed |
...
| FR-018 | The core module MUST export a shared execution-source resolver, with a documented contract stating that a replay-configured cassette counts as a configured execution source, shipped with its own unit tests independent of any wave-2 command. | Proposed |
...
| NFR-007 | The invariant guard suite (including the new size lint) MUST stay inside its existing combined performance budget. | Combined NI-001/002/003 (+ size lint) runtime ≤ the suite's existing 2000 ms budget. | Proposed |
...
| C-004 | Cassette provenance never stores API key values, `apiKeyEnv` values, or full endpoint URLs — hostname only, matching the client's existing error-hygiene convention (`hostnameOf` in `client.ts`). | Accepted |
| C-005 | The execution-source resolver ships in this mission, exported from core, for wave-2 (#100/#101/#102) to consume without modification. | Accepted |
| C-006 | The sequential-only, per-suite model-call execution model (no `Promise.all` around any model call, confirmed across all seven behavioral runners) is preserved; this mission introduces no concurrent model-call path, since ordinal keying depends on strict sequencing. | Accepted |
...
| SC-006 | Wave-2 missions (#100/#101/#102) can import the cassette core types, decorator, and execution-source resolver without any change to `src/core/`. |
...
- **Exchange** — one recorded request/response pair: request hash, ordinal, the
  fully-built request (`messages[]`, `opts`), the response, and minimal provenance
  (requested model, endpoint hostname).
- **Request hash** — the canonical-JSON hash of the fully-built, post-transform
  request; the keying identity for an exchange.
...
- **Execution-source resolver** — a core-exported function/contract stating that a
  replay-configured cassette is a configured execution source, consumed by wave-2
  commands.
...
- **Blocks**: M17 (#100), M18 (#101), M19 (#102) — the format, decorator, and
  execution-source resolver they all consume — and transitively M20 (#103).
...
- **Assumption**: all seven behavioral runners execute cases and runs
  sequentially — no `Promise.all` around any model call — confirmed in the current
  tree; the ordinal keying scheme depends on this remaining true.
...
- **Assumption**: `node:crypto`'s `createHash` (already used in
  `src/adapters/spec-kitty-profile/projection.ts` and
  `src/adapters/a2a/signature.ts`) is available and sufficient for request hashing
