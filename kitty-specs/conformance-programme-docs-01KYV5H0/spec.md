...
- No index over the six rubric documents; no mechanism catching citation drift (muster#80: 5 of 6 sampled code anchors in `skills-trigger-taxonomy.md` are currently stale).
...
- **FR-002**: A rubric index MUST exist (`docs/rubric/index.md` or equivalent) listing all six current rubric documents (`crosslayer-contradiction-gate.md`, `memory-utilization-taxonomy.md`, `skills-trigger-taxonomy.md`, `sop-rule-taxonomy.md`, `spec-kitty-behavioral-axes.md`, `spec-kitty-profile-taxonomy.md`), each entry stating which adapter/check(s) cite it. A mechanical drift-check script (`scripts/check-rubric-citations.mjs`) MUST parse every `` `symbolName` (`file:line[-line]`) `` citation pattern (the convention already in use, e.g. `docs/rubric/skills-trigger-taxonomy.md:89`) from every file under `docs/rubric/*.md`, resolve the file, confirm the line range is in-bounds, and confirm the named symbol/string literally appears within that line range in the current tree; it MUST also fail if any file under `docs/rubric/*.md` has zero index entries pointing at it. Per the resolved decision, this FR also mechanically repairs `skills-trigger-taxonomy.md`'s five stale anchors (muster#80) as a byproduct of building the checker — pointer correction only, no rubric judgment/severity content changes, so it does not breach the "no rubric content changes beyond the index" scope guard.
  - *Verification*: `node scripts/check-rubric-citations.mjs`.
  - *Expected exit code*: `0` once repaired (post-FR-002); the *falsification* run is `git stash` the repair, then re-run — expected `1`, naming `trigger.ts:200-206`, `trigger.ts:430-435`, `trigger.ts:438-440`, `tests/cts/skills-suite.test.ts:231-307`, and `tests/cts/skills-suite.test.ts:312-403` as stale (their real current locations are `trigger.ts:237`, `~472-473`, `~481-482`, `244-321`, `325-438` respectively). This falsification case is not hypothetical — it is the file's real, currently-shipped state at commit `16f0d34c3`.
...
- **FR-004**: The Starlight site nav (`site/astro.config.mjs`'s `sidebar` array) MUST be updated to include the three new pages, and `.github/workflows/site.yml` ("Deploy site") MUST build green with them present.
  - *Verification*: `cd site && pnpm build`.
  - *Expected exit code*: `0`.
  - *Falsification condition*: a broken internal link or an unregistered page under `site/src/content/docs/` that Starlight's build-time link checker flags exits non-zero.
...
- **SC-002**: `node scripts/check-register-schema.mjs docs/rubric/recorded-gaps.md` exits 0 with exactly 7 entries, all six schema fields populated per entry.
...
Seven entries populate FR-003 at merge time (all fields per the Key Entities schema; abbreviated here, full text belongs in `docs/rubric/recorded-gaps.md`):
...
- **WP01** — operator guide (`docs/guides/spec-kitty-conformance.md`), `examples/README.md` correction (FR-001, FR-005), `.env.example` caveat (FR-006).
- **WP02** — rubric index + drift-check script + anchor repair (FR-002), recorded-gaps register + schema-check script (FR-003), site nav update (FR-004).

`write_scope` for both WPs is confined to `docs/**`, `site/**`, `scripts/check-rubric-citations.mjs`, `scripts/check-register-schema.mjs`, and their `tests/**` counterparts (C-001). Both WPs are declared against the *same* lane (single_branch topology, per `spec-kitty agent mission create`'s recorded topology) — there is no sibling-lane isolation hazard here since both WPs share one worktree, but the `dependencies` for WP01 and WP02 must each still list every file their own manifests transitively reference (e.g., WP02's drift-check script references every file under `docs/rubric/*.md` and the source files each cites — `src/adapters/skills/trigger.ts`, `src/adapters/skills/types.ts`, `tests/cts/skills-suite.test.ts` — as read-only inputs, not as owned/written files).
