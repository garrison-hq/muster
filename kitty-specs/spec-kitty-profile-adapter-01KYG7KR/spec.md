...
|----|-------------|--------|
| FR-001 | New adapter `src/adapters/spec-kitty-profile/` with manifest type `{version, profilesDir, schemaPath, schemaSha, doctrineRoot, activationConfigPath?, projectionManifestPath?, cases[]}`; paths relative to the manifest file. *(Post-spec-gate correction: `schemaSha` added to the manifest type to resolve an internal contradiction between the Edge Cases SHA-pinning statement and D1's `agent-profile.schema.yaml@<SHA>` evidence, neither of which had a manifest field to carry the SHA; see Dependencies & Assumptions.)* *(Post-spec-gate correction: `doctrineRoot` added to the manifest type — FR-004/FR-005's directive/tactic/toolguide/styleguide resolution require a doctrine-tree root that is a sibling of, not implicit from, `profilesDir`; see Dependencies & Assumptions.)* | Proposed |
...
|----|-------------|-----------|--------|
| NFR-001 | The static check path (schema, handoff, reference, context-sources, profile-id, and projection-drift lints) runs fully offline with byte-stable deterministic output. | Zero network calls; identical bytes across repeated runs on the same machine in all cases, and additionally across machines for muster-local fixtures (`fixtures/skprofile/**`, `examples/skprofile/**`), for `muster skprofile run <manifest> --json`. *(Post-spec-gate correction: the unqualified "across machines" claim is false for a live SK installation's real `.kittify/agent_profiles_manifest.json`, which bakes in absolute, machine-specific `source_path`/`output_path` values (research.md R5); cross-machine variance is expected only where a real SK projection manifest's absolute paths are involved.)* | Proposed |
...
- **Design decision (post-spec-gate: `doctrineRoot` field)**: the manifest carries a `doctrineRoot` field (added to FR-001) naming the directory that contains the sibling `directives/`, `tactics/`, `toolguides/`, and `styleguides/` trees FR-004/FR-005 resolve references and context-sources against; verified against the real upstream layout, these trees are siblings of `agent_profiles/` (and therefore of `profilesDir`), never descendants of it. Rejected alternative: inferring `doctrineRoot` implicitly from `profilesDir`'s directory shape (e.g. assuming `profilesDir` sits at `<root>/agent_profiles/built-in`), rejected because it is fragile, undocumented magic that breaks the moment a muster-local fixture tree (C-004) doesn't mirror the upstream directory shape exactly — an explicit field is one line in the manifest and removes the ambiguity entirely, consistent with C-004's rule that muster-local fixtures never rely on an implicit fixed-offset convention.
