...
- Upstream schema evolution: since `schemaPath` is pinned to a SHA recorded in the manifest and in fixtures, an upstream schema change is a deliberate, reviewed version bump — never a silent behavior change underneath a fixed manifest.
...
|----|-------------|--------|
| FR-001 | New adapter `src/adapters/spec-kitty-profile/` with manifest type `{version, profilesDir, schemaPath, activationConfigPath?, projectionManifestPath?, cases[]}`; paths relative to the manifest file. | Proposed |
| FR-002 | Schema conformance: each `*.agent.yaml` validates against the referenced agent-profile schema via Ajv; findings carry the schema path + pinned SHA in `source`. | Proposed |
...
| C-004 | Fixtures are muster-local (a miniature profile set under `fixtures/skprofile/`), so muster CI never needs the SK repo. | Proposed |
